...
from holding import (HOLDING_EXTRA_PRODUCTS, holding_accessory_from_text,
                     holding_product_by_item)
...
def clean_delivery_notice(value):
    """기본 상시 배송문구를 제거하고 실제 전달사항만 보존한다."""
...
EXCEPTION_WORDS = ["수리", "롤스크린", "부속", "레일", "커버", "점보"]
...
class Master:
    def __init__(self, path):
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # 믹스 품목은 이름과 조합을 함께 보존한다. 현재 마스터의 믹스는
            # C자 투코드 또는 -원코드 형식이며, 없는 조합은 임의 생성하지 않는다.
            mm = re.match(
                r"^B(.+?)(-원코드)?\((\d{3}(?:\+\d{3})+)\)$", str(name))
            if mm:
                mix_name = mm.group(1)
    ...
    def find_holding_extra(self, name, client):
        row = HOLDING_EXTRA_PRODUCTS.get(name)
        if not row:
            return None
        out = dict(row)
        tier = CLIENT_INFO.get(client, (None, "출고I가"))[1]
...
def validate(order, M):
    ...
    if ("택배" in mode or "화물" in mode) and not str(delivery.get("주소") or "").strip():
        out.append(("yellow", "배송주소누락",
                    "택배/화물 주문인데 주소가 없습니다. 사전점검 부가정보의 주소를 확인해 주세요.", None))

    # 제이원/루임트 표의 수량과 좌·우 숫자는 행별/전체 합계로 교차검증한다.
    if client in {"JO", "RT", "DU"} and items:
    ...
    for i, it in enumerate(items, 1):
        ...
        if is_holding and not it.get("_holding_accessory"):
            ...
            from holding import holding_magnet_charge_qty
            if holding_magnet_charge_qty(it.get("_holding_operation") or it.get("수량")):
                ex = M.find_holding_extra("H추가비용(+자석바1)", client)
                tier = CLIENT_INFO.get(client, ('','출고I가'))[1]
                if not ex or not ex.get("단가"):
                    out.append(("yellow", "홀딩자석바단가확인",
                                f"자석바 추가비용 — {tier} 단가를 확인해 주세요", i))
                elif tier in (ex.get("_inferred_tiers") or set()):
                    out.append(("yellow", "홀딩자석바단가추정",
                                f"자석바 추가비용 {tier} {ex['단가']:,}원은 제공 자료에 직접 없어 임시 적용했습니다. 단가만 확인해 주세요", i))
        ...
        if not is_holding and h and handle_length is not None \
            ...
            out.append(("yellow", "손잡이길이차이",
                        ...
                        "100cm 이상 차이", i))
        _, adj = normalize_handle(it.get("손잡이길이")) if not is_holding else (None, False)
        if adj:
