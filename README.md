...
## 2026-09-18 변경 — SP 스페이스 손글씨 판독 (`tests/test_sp.py`)
- 주문번호는 `현장전달사항` 칸의 꺾쇠 안(`< 2-12 >`)에 있고 `주문일 - 동그라미 숫자` 형식이다.
  표 확대본만 보내던 때는 표본 10쪽에서 번호를 **하나도** 못 읽었다. **머리글 확대본**(위 30%)을
  따로 보내게 고쳐 10/10이 됐다. 꺾쇠 표기가 있으면 모델 판독보다 우선한다 (`extract._fix_sp_order_no`)
- 사진 입력 지원: 눕혀 찍힌 사진을 세우고(`extract.prepare_sp_image`),
  주문번호가 같은 장끼리 한 건으로 합친다 (`desktop_workflow.merge_sp_page_orders`)
- 수량 칸 `1/2`는 손잡이 분할이 아니라 **가로 2등분**. 나눈 창은 각각 청구한다
- 장부 상호 칸 `SP   (17)`, 기재사항1 = 발주번호+공통 / 기재사항2 = 창별,
  경영박사 적요 `피스/3-17/작은방/우미린아파트` 순서

### 판독 정확도 측정 (sp1·SP2·SP3 59쪽 × 2회 독립 판독)
| 항목 | 두 번 판독이 갈린 수 |
| --- | --- |
| 가로·세로 치수 | **0 / 148** |
| 창 개수 | 33 |
...
치수는 안정적이고, **손잡이 좌/우 방향이 가장 약하다**. 작대기를 이미지 처리로 세어
교차검증하는 방법은 오히려 맞는 판독을 틀렸다고 띄우는 일이 더 많아 채택하지 않았다.
...
- SP는 손잡이 좌/우 방향 판독이 불안정하다(위 측정표). 사전점검에서 방향을 확인하는 편이 안전하다
...
- `tests/test_sp.py` : SP 주문번호 보정·장부 표기·적요 순서·가로 2등분·여러 장 합치기
